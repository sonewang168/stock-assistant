...
async function handleCommand(message, userId) {
  ...
  // 🆕 預約買賣功能
  // 預約買 2330 550 2張 或 預約賣 6770 66 1張
  if (/^預約買\s*\d{4,6}/.test(msg) || /^預約賣\s*\d{4,6}/.test(msg)) {
    return await setTradeReservation(msg);
  }
  
  // 查看預約清單
  if (msg === '預約' || msg === '預約清單' || msg === '查預約') {
    return await getReservationList();
  }
  
  // 取消預約：取消預約 2330
  if (/^取消預約\s*\d{4,6}$/.test(msg)) {
    const stockId = msg.replace(/^取消預約\s*/, '').trim();
    return await cancelReservation(stockId);
  }
  ...
  // 🆕 修正賣出指令：修正賣出 股票代碼 新價格
  if (/^(修正賣出|改賣出|更正賣出)\s*\d{4,6}/.test(msg)) {
    return await correctSoldPrice(msg);
  }
  
  // 🆕 取消賣出指令：取消賣出 股票代碼
  if (/^(取消賣出|撤銷賣出)\s*\d{4,6}/.test(msg)) {
    return await cancelSold(msg);
  }
 ...
 * 格式：修正賣出 股票代碼 新價格
 */
async function correctSoldPrice(message) {
  try {
    const parts = message.replace(/^(修正賣出|改賣出|更正賣出)\s*/, '').trim().split(/\s+/);
    const stockId = parts[0];
    const newSoldPrice = parseFloat(parts[1]);
    
    if (!stockId || !/^\d{4,6}$/.test(stockId)) {
      return { type: 'text', text: '❌ 請輸入正確格式\n\n修正賣出 股票代碼 新價格\n例如：修正賣出 6770 64.5' };
    }
    
    if (!newSoldPrice || isNaN(newSoldPrice) || newSoldPrice <= 0) {
      return { type: 'text', text: '❌ 請輸入正確的新賣出價格\n\n修正賣出 股票代碼 新價格\n例如：修正賣出 6770 64.5' };
    }
    
    // 查詢該股票的已賣出紀錄
    const result = await pool.query(`
      SELECT * FROM holdings 
      WHERE stock_id = $1 AND user_id = 'default' AND is_sold = true 
      ORDER BY sold_date DESC, updated_at DESC 
      LIMIT 1
    `, [stockId]);
    
    if (result.rows.length === 0) {
      return { type: 'text', text: `❌ 找不到 ${stockId} 的已賣出紀錄` };
    }
    
    const holding = result.rows[0];
    const oldSoldPrice = parseFloat(holding.sold_price) || 0;
    const wonPrice = parseFloat(holding.won_price) || 0;
    const lots = parseInt(holding.lots) || 0;
    const oddShares = parseInt(holding.odd_shares) || 0;
    const totalShares = lots * 1000 + oddShares;
    ...
    await pool.query(`
      UPDATE holdings 
      SET sold_price = $1, updated_at = NOW() 
      WHERE id = $2
    `, [newSoldPrice, holding.id]);
    
    // 計算新的損益
    const FEE_RATE = 0.001425;
    const TAX_RATE = 0.003;
    const costTotal = wonPrice * totalShares;
    const newSoldTotal = newSoldPrice * totalShares;
    const buyFee = Math.round(costTotal * FEE_RATE);
    const sellFee = Math.round(newSoldTotal * FEE_RATE);
    const tax = Math.round(newSoldTotal * TAX_RATE);
    const netProfit = newSoldTotal - costTotal - buyFee - sellFee - tax;
    const isProfit = netProfit >= 0;
    
    // 取得股票名稱
    const twInfo = twStocks.getStockInfo(stockId);
    const stockName = holding.stock_name || twInfo?.name || stockId;
    
    return {
      type: 'flex',
      altText: `✅ ${stockName} 賣出價格已修正`,
      contents: {
        type: 'bubble',
        size: 'kilo',
        header: {
          type: 'box', layout: 'vertical',
          contents: [
            { type: 'text', text: `✅ ${stockName}(${stockId})`, color: '#ffffff', size: 'lg', weight: 'bold' },
            { type: 'text', text: '賣出價格已修正', color: '#ffffffcc', size: 'sm', margin: 'sm' }
          ],
        ...
        body: {
          ...
          contents: [
            { type: 'box', layout: 'horizontal', contents: [
              { type: 'text', text: '原賣出價', size: 'sm', color: '#888888', flex: 2 },
              { type: 'text', text: `$${oldSoldPrice}`, size: 'sm', align: 'end', flex: 2, decoration: 'line-through' }
            ]},
            { type: 'box', layout: 'horizontal', margin: 'sm', contents: [
              { type: 'text', text: '新賣出價', size: 'sm', color: '#333333', flex: 2, weight: 'bold' },
              { type: 'text', text: `$${newSoldPrice}`, size: 'sm', align: 'end', flex: 2, weight: 'bold', color: '#27AE60' }
            ]},
            { type: 'separator', margin: 'lg' },
            { type: 'box', layout: 'horizontal', margin: 'lg', contents: [
              { type: 'text', text: '修正後淨損益', size: 'sm', color: '#333333', flex: 2 },
              { type: 'text', text: `${isProfit ? '+' : ''}$${netProfit.toLocaleString()}`, size: 'md', align: 'end', flex: 2, weight: 'bold', color: isProfit ? '#D32F2F' : '#388E3C' }
            ]}
          ],
          paddingAll: '15px'
        }
    ...
    console.error('修正賣出價格錯誤:', error);
    return { type: 'text', text: `❌ 修正失敗：${error.message}` };
  }
}
 ...
 * 格式：取消賣出 股票代碼
 */
async function cancelSold(message) {
  try {
    const stockId = message.replace(/^(取消賣出|撤銷賣出)\s*/, '').trim();
    ...
    await pool.query(`
      UPDATE holdings 
      SET is_sold = false, sold_price = NULL, sold_date = NULL, updated_at = NOW() 
      WHERE id = $1
    `, [holding.id]);
    ...
    return {
      ...
      contents: {
        ...
        header: {
          ...
          contents: [
            { type: 'text', text: `🔄 ${stockName}(${stockId})`, color: '#ffffff', size: 'lg', weight: 'bold' },
            { type: 'text', text: '已恢復為持股中', color: '#ffffffcc', size: 'sm', margin: 'sm' }
          ],
        ...
        body: {
          type: 'box', layout: 'vertical',
          contents: [
            { type: 'text', text: '此筆賣出紀錄已取消', size: 'sm', color: '#666666' },
            { type: 'text', text: '股票已恢復到持股清單中', size: 'sm', color: '#666666', margin: 'sm' },
            { type: 'separator', margin: 'lg' },
            { type: 'text', text: '💡 如需重新賣出，請輸入：', size: 'xs', color: '#888888', margin: 'lg' },
            { type: 'text', text: `賣出 ${stockId} 賣出價格`, size: 'xs', color: '#888888', margin: 'sm' }
          ],
          paddingAll: '15px'
        }
      }
...
async function getRealtimeWebLinkWithHoldings() {
  ...
  const webLinkBubble = {
    ...
    header: {
      ...
      contents: [
        { type: 'box', layout: 'vertical', flex: 3,
          contents: [
            { type: 'text', text: '📊 即時報價看板', size: 'lg', weight: 'bold', color: '#60A5FA' },
            { type: 'text', text: '專業股票分析系統', size: 'xxs', color: '#94A3B8', margin: 'xs' }
          ]
        },
    ...
    body: {
      ...
      backgroundColor: '#111827',
      contents: [
        { type: 'text', text: '🏠 台股專業分析', size: 'sm', weight: 'bold', color: '#E2E8F0' },
        { type: 'separator', margin: 'sm', color: '#2d3a4f' },
        { type: 'box', layout: 'vertical', margin: 'sm', spacing: 'xs',
          contents: [
            { type: 'text', text: '📈 即時股價 K 線圖', size: 'xxs', color: '#10B981' },
            { type: 'text', text: '📊 技術指標 RSI/KD/MACD', size: 'xxs', color: '#3B82F6' },
            { type: 'text', text: '🌊 艾略特波浪分析', size: 'xxs', color: '#8B5CF6' },
            { type: 'text', text: '🤖 三 AI 智能分析', size: 'xxs', color: '#EC4899' }
          ]
        },
        { type: 'box', layout: 'horizontal', margin: 'md',
          contents: [
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      for (const row of result.rows) {
        try {
          ...
          const stockName = row.stock_name || stockData?.name || row.stock_id;
          const currentPrice = parseFloat(stockData?.price) || 0;
          const prevClose = parseFloat(stockData?.prevClose) || parseFloat(stockData?.yesterdayPrice) || currentPrice;
          const buyPrice = parseFloat(row.buy_price) || 0;
          const totalShares = (parseInt(row.lots) || 0) * 1000 + (parseInt(row.odd_shares) || 0);
          ...
          // 計算損益
          const profit = (currentPrice - buyPrice) * totalShares;
          const profitPercent = buyPrice > 0 ? ((currentPrice - buyPrice) / buyPrice * 100) : 0;
          const isProfitable = profit > 0;
          
          totalHoldings += currentPrice * totalShares;
          totalProfit += profit;
          
          // 顯示格式：台積電(2330) 昨:1805 → 今:1775 ▼-1.66%
          const priceColor = isUp ? '#EF4444' : (isFlat ? '#94A3B8' : '#22C55E');
          const arrow = isUp ? '▲' : (isFlat ? '－' : '▼');
          const profitColor = isProfitable ? '#EF4444' : '#22C55E';
          
          holdingsContents.push({
            type: 'box',
            layout: 'vertical',
            paddingAll: '8px',
            backgroundColor: '#1a1f2e',
            ...
            contents: [
              { type: 'box', layout: 'horizontal',
                contents: [
                  { type: 'text', text: `${stockName}`, size: 'sm', weight: 'bold', color: '#E2E8F0', flex: 3 },
                  { type: 'text', text: row.stock_id, size: 'xxs', color: '#64748B', align: 'end', flex: 2 }
                ]
              },
              { type: 'box', layout: 'horizontal', margin: 'xs',
                contents: [
                  { type: 'text', text: `昨 ${prevClose.toFixed(0)}`, size: 'xxs', color: '#64748B', flex: 2 },
                  { type: 'text', text: `→`, size: 'xxs', color: '#64748B', flex: 1, align: 'center' },
                  { type: 'text', text: `${currentPrice.toFixed(0)}`, size: 'sm', weight: 'bold', color: priceColor, flex: 2, align: 'end' },
                  { type: 'text', text: `${arrow}${Math.abs(changePercent).toFixed(2)}%`, size: 'xxs', color: priceColor, flex: 2, align: 'end' }
                ]
              },
              { type: 'box', layout: 'horizontal', margin: 'xs',
                contents: [
                  { type: 'text', text: `成本 ${buyPrice.toFixed(0)}`, size: 'xxs', color: '#64748B', flex: 3 },
                  { type: 'text', text: `${isProfitable ? '+' : ''}${profitPercent.toFixed(1)}%`, size: 'xxs', weight: 'bold', color: profitColor, flex: 2, align: 'end' }
                ]
              }
            ]
          });
        } catch (e) {
          console.error(`獲取 ${row.stock_id} 即時價格失敗:`, e.message);
        }
      }
      
      if (holdingsContents.length > 0) {
        const totalProfitPercent = totalHoldings > 0 ? (totalProfit / (totalHoldings - totalProfit) * 100) : 0;
        ...
        holdingsBubble = {
          ...
          header: {
            ...
            paddingAll: '12px',
            contents: [
              { type: 'text', text: '💼 持股即時報價', size: 'lg', weight: 'bold', color: '#F59E0B', flex: 3 },
              { type: 'text', text: isTotalProfit ? '📈' : '📉', size: 'xl', flex: 0 }
            ]
          },
          body: {
            type: 'box',
            ...
            contents: [
              // 總計
              { type: 'box', layout: 'horizontal', paddingAll: '6px', backgroundColor: isTotalProfit ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)', cornerRadius: '6px',
                contents: [
                  { type: 'text', text: '總損益', size: 'xs', color: '#94A3B8', flex: 2 },
                  { type: 'text', text: `${isTotalProfit ? '+' : ''}${(totalProfit/1000).toFixed(1)}K`, size: 'sm', weight: 'bold', color: isTotalProfit ? '#EF4444' : '#22C55E', flex: 2, align: 'end' },
                  { type: 'text', text: `${isTotalProfit ? '+' : ''}${totalProfitPercent.toFixed(1)}%`, size: 'xs', color: isTotalProfit ? '#EF4444' : '#22C55E', flex: 2, align: 'end' }
                ]
              },
              { type: 'separator', margin: 'sm', color: '#2d3a4f' },
              // 持股列表
          ...
          footer: {
            ...
            contents: [
              ...
              { type: 'button', style: 'secondary', height: 'sm', flex: 1,
                action: { type: 'message', label: '📊 摘要', text: '持股摘要' }
              }
            ]
          }
        };
    ...
    console.error('獲取持股即時報價失敗:', e.message);
  }
  
  // 組合 Carousel
  const bubbles = [webLinkBubble];
  if (holdingsBubble) {
    bubbles.push(holdingsBubble);
  }
...
async function setTradeReservation(message) {
  try {
    ...
    // 提取張數和股數
    const lotsMatch = cleanMsg.match(/(\d+)\s*張/);
    const sharesMatch = cleanMsg.match(/(\d+)\s*股/);
    
    if (lotsMatch) {
      lots = parseInt(lotsMatch[1]);
      cleanMsg = cleanMsg.replace(/\d+\s*張/, '').trim();
    }
    if (sharesMatch) {
      oddShares = parseInt(sharesMatch[1]);
      cleanMsg = cleanMsg.replace(/\d+\s*股/, '').trim();
    }
    ...
    if (currentPrice > 0) {
      ...
      if (ratio > 10 || ratio < 0.1) {
        return {
          type: 'text',
          text: `⚠️ 目標價 ${targetPrice} 與現價 ${currentPrice} 差距過大\n\n請確認價格是否正確，如確定請再輸入一次`
        };
      }
    }
    ...
    const existing = await pool.query(
      `SELECT * FROM trade_reservations WHERE stock_id = $1 AND trade_type = $2 AND is_triggered = false`,
      [stockId, isBuy ? 'buy' : 'sell']
    );
    
    if (existing.rows.length > 0) {
      // 更新現有預約
      await pool.query(
        `UPDATE trade_reservations SET target_price = $1, lots = $2, odd_shares = $3, current_price_at_set = $4, created_at = NOW() 
         WHERE stock_id = $5 AND trade_type = $6 AND is_triggered = false`,
        [targetPrice, lots, oddShares, currentPrice, stockId, isBuy ? 'buy' : 'sell']
      );
    } else {
      // 插入新預約
      await pool.query(
    ...
    const priceDiff = currentPrice > 0 ? targetPrice - currentPrice : 0;
    ...
    console.error('設定預約錯誤:', error);
    return { type: 'text', text: `❌ 設定預約失敗: ${error.message}` };
  }
}
...
async function getReservationList() {
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM trade_reservations WHERE is_triggered = false ORDER BY created_at DESC`
    );
    ...
    console.error('查詢預約清單錯誤:', error);
    return { type: 'text', text: `❌ 查詢預約清單失敗: ${error.message}` };
  }
}
 ...
 * 🆕 預約買賣功能 - 取消預約
 */
async function cancelReservation(stockId) {
  try {
    const result = await pool.query(
      `DELETE FROM trade_reservations WHERE stock_id = $1 AND is_triggered = false RETURNING *`,
      [stockId]
    );
    
    if (result.rowCount === 0) {
      return { type: 'text', text: `❌ 找不到 ${stockId} 的預約單` };
    }
    
    const stockName = result.rows[0]?.stock_name || stockId;
    
    return {
      type: 'flex',
      altText: `✅ 已取消 ${stockName} 的預約`,
      contents: {
        type: 'bubble',
        size: 'kilo',
        body: {
          ...
          paddingAll: '20px',
          contents: [
            { type: 'text', text: '✅ 預約已取消', size: 'lg', weight: 'bold', color: '#10B981', align: 'center' },
            { type: 'text', text: `${stockName} (${stockId})`, size: 'md', color: '#E5E7EB', align: 'center', margin: 'md' },
            { type: 'text', text: `已刪除 ${result.rowCount} 筆預約`, size: 'sm', color: '#9CA3AF', align: 'center', margin: 'sm' }
          ]
        },
        footer: {
          type: 'box',
          ...
          contents: [
            { type: 'button', style: 'secondary', height: 'sm',
              action: { type: 'message', label: '📋 查看預約清單', text: '預約' }
            }
          ]
        }
      }
